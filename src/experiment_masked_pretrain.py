...
def main():
    p=argparse.ArgumentParser();p.add_argument('--pretrain-epochs',type=int,default=25);p.add_argument('--finetune-epochs',type=int,default=80);p.add_argument('--seed',type=int,default=42);p.add_argument('--test',action='store_true');a=p.parse_args();set_seed(a.seed);d=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    m=PretrainedEEGClassifier().to(d);m.encoder.load_state_dict(ae.encoder.state_dict());opt=torch.optim.AdamW(m.parameters(),lr=1e-3,weight_decay=.02);sched=torch.optim.lr_scheduler.ReduceLROnPlateau(opt,factor=.5,patience=3,min_lr=1e-5);ce=nn.CrossEntropyLoss(label_smoothing=.1);best=float('inf');stale=0;path=os.path.join(ROOT,'models','checkpoints',f'masked_pretrained_eeg_seed{a.seed}.pth')
